...
CREATE TABLE users (
    ...
    role VARCHAR(50) NOT NULL CHECK (role IN ('CITIZEN', 'BUSINESS', 'INSPECTOR', 'CONTROLLER')),
...
CREATE TABLE businesses (
    ...
    turnover_category VARCHAR(50) DEFAULT 'MICRO'
);
...
CREATE TABLE inspections (
    ...
    inspection_type VARCHAR(50) CHECK (inspection_type IN ('ROUTINE_RAID', 'CITIZEN_COMPLAINT', 'SUPPLY_CHAIN_LEAD')),
...
CREATE TABLE violations (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    inspection_product_id UUID REFERENCES inspection_products(id),
...
CREATE TABLE notices (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    inspection_id UUID REFERENCES inspections(id),
    notice_type VARCHAR(50) CHECK (notice_type IN ('IMPROVEMENT_NOTICE', 'SEIZURE_LIST', 'PANCHANAMA', 'COMPOUNDING_ORDER')),
    stage INT NOT NULL,
    document_url TEXT NOT NULL,
    digital_signature_hash TEXT,
    controller_approved BOOLEAN DEFAULT FALSE,
    compounding_fee NUMERIC(10, 2),
    payment_status VARCHAR(50) DEFAULT 'UNPAID' CHECK (payment_status IN ('UNPAID', 'PAID', 'EXEMPTED', 'REFERRED_TO_COURT')),
...
CREATE TABLE complaints (
    ...
    status VARCHAR(50) DEFAULT 'SUBMITTED' CHECK (status IN ('SUBMITTED', 'VERIFIED', 'RAIDED', 'COMPOUNDED', 'REJECTED')),
    bounty_amount NUMERIC(10, 2) DEFAULT 0.0,
    bounty_paid_at TIMESTAMP WITH TIME ZONE
);
```
...
| **Phase 5** | **Relational Persistence** | SQLite/PostgreSQL schema layer linking inspections, violations, and notices | 🔄 Next Step |
